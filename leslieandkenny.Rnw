...
$$
y_i\sim\mathrm{overdispersed Binomial}(30, p_i, \omega),
$$
$$
...
observation \(i\),
\item \(\omega\) is an overdispersion parameter, so that the standard
deviation of the response in plot \(i\) is \(\sqrt{30\omega p_i(1-p_i)}\).
\end{itemize}
...
#Warning message:
#In tableplot_checkCols(substitute(sortCol), colNames) :
#  Sorting on multiple columns not supported anymore. Only first column is taken.
