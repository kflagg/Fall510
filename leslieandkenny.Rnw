...
fert.sim1$uninfected <- with(fert.sim1, total - infected)
@
...
glmQBin.noSNMS <- glm(cbind(infected, uninfected) ~ block*variety +
...
noSNMS3way <- glmer(cbind(infected, uninfected) ~ variety*n.trt*status2 +
...
glm3way <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt*status2,
...
noVar1 <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt*status2,
...
onlyINOC <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt,
                family = quasibinomial, data = fert.sim1, subset = status=="INOC")

onlyCNTL <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt,
...
glm3way2 <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt*status2,
                family = quasibinomial, data = fert2)
summary(glm3way2)

glm2way2 <- glm(cbind(infected, uninfected) ~ block*variety + (variety+n.trt+status2)^2,
                family = quasibinomial, data = fert2)
summary(glm2way2)
...
<<glmm1,eval=FALSE>>=
glmm3way <- glmer(cbind(infected, uninfected) ~ variety*n.trt*status2 + (1|block/variety),
                  control = glmerControl(optimizer = "bobyqa"),
...
glmmPlot <- glmer(cbind(infected, uninfected) ~ variety*n.trt*status2 + (1|plot),
                  control = glmerControl(optimizer = "bobyqa",
                                         optCtrl = list("maxfun" = 20000)),
                  family = binomial, data = fert2)
glmmVar <- glmer(cbind(infected, uninfected) ~ variety*n.trt*status2 +
                   (1|block/variety),
                 control = glmerControl(optimizer = "bobyqa",
                                        optCtrl = list("maxfun" = 20000)),
                 family = binomial, data = fert2)
...
glmBin <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt*status2,
              family = binomial, data = fert2)
# Warning message:
...
glmQBin <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt*status2,
               family = quasibinomial, data = fert2)
# No warnings
# Residual deviance: 427.939  on 125  degrees of freedom
...
glmmVarPlot <- glmer(cbind(infected, uninfected) ~ variety*n.trt*status2 +
...
glmmRow <- glmer(cbind(infected, uninfected) ~ variety*n.trt*status2 +
                   (1|block/row),
                 control = glmerControl(optimizer = "bobyqa",
                                        optCtrl = list("maxfun" = 20000)),
                 family = binomial, data = fert2)
...
glmmRowPlot <- glmer(cbind(infected, uninfected) ~ variety*n.trt*status2 +
                       (1|block/row) + (1|plot),
                     control = glmerControl(optimizer = "bobyqa",
                                            optCtrl = list("maxfun" = 20000)),
                     family = binomial, data = fert2)
...
glmmVarS <- glmer(cbind(infected, uninfected) ~ variety*n.trt*status2 +
                    (variety|block),
                  control = glmerControl(optimizer = "bobyqa",
                                         optCtrl = list("maxfun" = 20000)),
                  family = binomial, data = fert2)
...
noSNMS2way <- glmer(cbind(infected, uninfected) ~ variety*n.trt + variety*status2 +
...
\noindent Warning: the results reported in this section are from simulated data. This section is meant to be used as an example that can be followed when working with the real dataset. \\
...
\begin{quote}
There is no evidence that the interaction between variety and nitrogen application time differs between inoculated and control plots (p-value$=0.406$ from F-stat$=1.05$ on $8$ and $125$ df).
\end{quote}
...
glm2way <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt
...
\begin{enumerate}
\item \verb+n.trt:status2+ ``There is no evidence that the relationship between nitrogen application time and the odds of virus infection differs between inoculated and control plots after controlling for all other two way interactions and main effects in the model (p-value$=0.386$ from F-stat$=0.959$ on $2$ and $133$ df).
\item \verb+variety:status2+ ``There is no evidence that the relationship between variety and the odds of virus infection differs between inoculated and control plots after controlling for all other two way interactions and main effects in the model (p-value$=0.302$ from F-stat$=1.23$ on $4$ and $133$ df).
\item \verb+variety:n.trt+ ``There is no evidence that the relationship between nitrogen application time and the odds of virus infection depends on variety after controlling for all other two way interactions and main effects in the model (p-value$=0.465$ from F-stat$=0.967$ on $8$ and $133$ df).
\end{enumerate}
...
\begin{quote}
There is no evidence that the relationship between nitrogen application time and the odds of virus infection depends on variety after controlling for all main effects and the two way interaction between variety and inoculation status (p-value$=0.440$ from F-stat$=0.999$ on $8$ and $135$ df).
\end{quote}
<<refitno2way, echo=FALSE, results='asis'>>=
glm2way2 <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt
               + variety*status2,
               family = quasibinomial, data = fert.sim1)
...
glm2way3 <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt,
               family = quasibinomial, data = fert.sim1)
...
\begin{quote}
There is no evidence that the odds of virus infection depends on nitrogen application time after controlling for block, variety, inoculation status, and the $block*variety$ interaction (p-value$=0.633$ from F-stat$=0.459$ on $2$ and $147$ df).
\end{quote}

<<glmnointeractions, echo=TRUE, results='hide'>>=
glmnointeractions <- glm(cbind(infected, uninfected) ~ block*variety + variety + n.trt + status2,
               family = quasibinomial, data = fert.sim1)
...
glm.no3way <- glm(cbind(infected, uninfected) ~ block*variety + variety*n.trt + n.trt*status2 + variety*status2,
               family = quasibinomial, data = fert.sim1)
#drop in dev F-test
#((196-185)/8)/(185/125)
#0.929
...
\begin{quote}
For inoculated plants of variety $1$, the odds of virus infection when nitrogen is applied in the early spring is estimated to be $3.04$ times the odds of virus infection when nitrogen is applied in the fall, with a $95\%$ likelihood-based confidence interval from  $0.264$ to $136.6$ times. For inoculated plants of variety $1$, the odds of virus infection when nitrogen is applied in the late spring is estimated to be $5.13$ times the odds of virus infection when nitrogen is applied in the fall, with a $95\%$ likelihood-based confidence interval from $0.604$ to $218.4$ times.
\end{quote}
...
glm.refvar3 <- glm(cbind(infected, uninfected) ~ block/var3 + var3*n.trt*status2,
               family = quasibinomial, data = fert.sim1)
#interpret results
...
#then fit model with var3
glm.refvar3cntl <- glm(cbind(infected, uninfected) ~ block/var3 + var3*n.trt*statuscntl,
               family = quasibinomial, data = fert.sim1)
#interpret results
...
\begin{quote}
For inoculated plants of variety $3$, the odds of virus infection when nitrogen is applied in the early spring is estimated to be about equal to the odds of virus infection when nitrogen is applied in the fall, with a $95\%$ likelihood-based confidence interval from  $0.174$ to $5.763$ times. For inoculated plants of variety $3$, the odds of virus infection when nitrogen is applied in the late spring is estimated to be $1.26$ times the odds of virus infection when nitrogen is applied in the fall, with a $95\%$ likelihood-based confidence interval from $0.252$ to $6.910$ times.
\end{quote}

\noindent We would describe the nitrogen application effect for non-inoculated plants of variety $3$ as follows:
\begin{quote}
For non-inoculated plants of variety $3$, the odds of virus infection when nitrogen is applied in the early spring is estimated to be $2.49$ times the odds of virus infection when nitrogen is applied in the fall, with a $95\%$ likelihood-based confidence interval from  $0.817$ to $8.965$ times. For non-inoculated plants of variety $3$, the odds of virus infection when nitrogen is applied in the late spring is estimated to be $1.54$ times the odds of virus infection when nitrogen is applied in the fall, with a $95\%$ likelihood-based confidence interval from $0.444$ to $5.87$ times.
\end{quote}
