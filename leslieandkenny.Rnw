...
that would cause it to have a much lower infrection rate. \textbf{If and
only if it is justified}, Nar could omit all of the data for this treatment
or variety when fitting the model. The scope of inference for this model
must then be limited to varieties and treatments that have a nonzero
infection rate.
...
the model should be modyfied to account for this.
...
<<models1,warning=FALSE,results='hide',cache=TRUE>>=
glmm3way <- glmer(cbind(infected, total) ~ variety*n.trt*status2 + (1|block/row),
                  control = glmerControl(optimizer = 'bobyqa', optCtrl = list(maxfun = 20000)),
                  family = binomial, data = fert.sim1)
@

We noticed that the SNMS variety had 27 out of 36 counts that were zeros.
Code like the following

<<models2,warning=FALSE,results='hide',cache=TRUE>>=
...
<<models3,warning=FALSE,results='hide',cache=TRUE>>=
...
glmm2way <- glmer(cbind(infected, total) ~ variety*n.trt + variety*status2 +
                    n.trt*status2 + (1|block/row),
                    control = glmerControl(optimizer = 'bobyqa'),
                  family = binomial, data = fert2)
#summary(glmm2way)
...
<<plots1,eval=FALSE,echo=FALSE,warning=FALSE,message=FALSE,out.height='0.5\\linewidth',fig.height=5>>=
par(mfrow = c(1, 2))
plot(resid(glmm3way) ~ predict(glmm3way), col = variety, data = fert.sim1)
plot(resid(glmm3way) ~ fitted(glmm3way), col = variety, data = fert.sim1)
legend('topright', pch = 1, col = 1:5,
       legend = levels(fert.sim1$variety))
@
...
<<glmmodel1,cache=FALSE>>=
glm1 <- glm(cbind(infected, total) ~ variety*n.trt*status2 + row,
            family = quasibinomial, data = fert.sim1)
glm2 <- glm(cbind(infected, total) ~ block + variety*n.trt*status2,
            family = quasibinomial, data = fert.sim1)
glm3 <- glm(cbind(infected, total) ~ block*variety + variety*n.trt*status2,
            family = quasibinomial, data = fert.sim1)
glm4 <- glm(cbind(infected, total) ~ block/row + variety*n.trt*status2,
            family = quasibinomial, data = fert.sim1)
glm5 <- glm(cbind(infected, total) ~ block/variety + variety*n.trt*status2,
            family = quasibinomial, data = fert.sim1)

glmnointMF <- glm(cbind(infected, total) ~ block/variety + variety*n.trt*status2,
            family = quasibinomial, data = fert2)
glmmnointMF <- glmer(cbind(infected, total) ~ variety + variety*n.trt*status2 + (1|block/row),
            family = binomial, data = fert.sim1)
@
...
\noindent We created variable with a unique level for each row, since row
it was not included in the dataset. We also changed the baseline levels
of \verb|n.application| and \verb|status|.
<<fakepre,eval=FALSE>>=
@
