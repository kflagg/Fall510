...
<<separation,echo=FALSE,out.width='0.6\\linewidth',fig.width=6>>=
...
arise while analyzing Nar's data. The examples all use simulated datasets.
...
\noindent We noticed that infection counts of 0 were recorded in 67 of the
...
be needed for the more resistant population. \\
...
# Tableplot is confusing. Maybe omit this?
fert.sim1$v.trt <- with(fert.sim1, interaction(status2, n.trt, variety))
tableplot(fert.sim1, select = c(infected, variety, block, n.trt, status, v.trt),
          nBins = 30, sortCol = "v.trt")
@
\end{figure}
...
$$
$$
\mathrm{logit}(p_i)=\mu+\alpha_{b[i]}+\beta_{v[i]}+\gamma_{t[i]}+\delta_{j[i]}
+(\alpha\beta)_{b[i],v[i]}+(\beta\gamma)_{v[i],t[i]}+(\beta\delta)_{v[i],j[i]}
+(\gamma\delta)_{t[i],j[i]}+(\beta\gamma\delta)_{v[i],t[i],j[i]}
$$
where
\begin{itemize}
